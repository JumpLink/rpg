...
return (async () => {
  try {
    ...
    await page.screenshot({ path: "test/.tmp/loaded.png" });

    const start = await page.$("#excalibur-play");
    await start.click();
    await page.screenshot({ path: "test/.tmp/play.png" });

    await browser.close();

    imageMatch("./test/images/expected-loaded.png", "./test/.tmp/loaded.png");
    imageMatch("./test/images/expected-play.png", "./test/.tmp/play.png");
